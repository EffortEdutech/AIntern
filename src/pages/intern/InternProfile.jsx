 ...
 *
 * Profile now stays focused on account, pass, supervisor review settings,
 * and AI keys. Report and template controls live in Report Center.
 *
...
export default function InternProfile() {
  ...
  const chooseModel = async (provider, model) => {
    ...
    if (res.success) {
      setSavedKeys((keys) => keys.map((k) => (
        k.provider === provider ? { ...k, model: model || null } : k
      )));
      ...
      <div className="p-4 space-y-6">
        ...
        <section className="bg-white rounded-xl border border-gray-200 p-5 space-y-3">
          ...
          <p className="text-xs text-gray-500">
            Add your own AI key for unlimited polishing, or use the built-in AI
            with a monthly quota. Keys are encrypted and never shown again.
          </p>

          {savedKeys.length > 0 && (
            <ul className="space-y-2">
              {savedKeys.map((k) => (
                <li key={k.provider} className="rounded-lg border border-gray-200 px-3 py-2 space-y-2">
                  <div className="flex items-center justify-between">
                    <span className="text-sm text-gray-700 capitalize">
                      {k.provider} - saved key
                    </span>
                    <button

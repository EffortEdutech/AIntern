-- AIntern - 011_freeze_report_center_template
--
-- Re-create create_report_snapshot() so Weekly/Monthly official versions
-- freeze the selected Report Center template JSON into report_versions.content.
-- Verification remains unchanged: >=1 approved entry and 0 pending entries in
-- the period. The frozen template is presentation metadata only.
...
begin
  ...
  -- Freeze the selected Report Center template for Weekly/Monthly reports.
  -- Custom imported templates live in internships.metadata.report_template_defs;
  -- standard defaults are mirrored here so old versions export consistently
  -- even if the intern changes their active template later.
